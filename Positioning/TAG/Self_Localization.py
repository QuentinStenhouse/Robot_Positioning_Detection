...
It requires the following modules: DW1000, DW1000Constants and monotonic. Test class
"""
...
def loop():
    global sentAck, receivedAck, timePollAckSentTS, timePollReceivedTS, timePollSentTS, timePollAckReceivedTS, timeRangeReceivedTS, protocolFailed, data, expectedMsgId, timeRangeSentTS, Anchor, d1,d2,d3,x
    ...
    if receivedAck:
        ...
        if msgId == C.POLL:
            ...
            if protocolFailed == False:
                ...
                if Anchor == 1:
                   DW1000.setPreambleCode(C.PREAMBLE_CODE_64MHZ_10)
                   DW1000.commitConfiguration()
                   Anchor = 2
                   d1 = float("%0.2f"%distance)
                   print("ANCHOR 1")
                   time.sleep(0.0001)
                elif Anchor == 2:
                   Anchor = 3
                   DW1000.setPreambleCode(C.PREAMBLE_CODE_64MHZ_11)
                   DW1000.commitConfiguration()
                   d2 = float("%0.2f"%distance)
                   print("ANCHOR 2")
                   time.sleep(0.0001)
                elif Anchor == 3:
                   DW1000.setPreambleCode(C.PREAMBLE_CODE_64MHZ_9)
                   DW1000.commitConfiguration()
                   Anchor = 1
                   d3 = float("%0.2f"%distance)
                   print("ANCHOR 3")
                   time.sleep(0.0001)
            else:
                transmitRangeFailed()
